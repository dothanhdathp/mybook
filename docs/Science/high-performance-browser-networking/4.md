...
- Chứng chỉ igvita.com được ký bởi Máy chủ trung gian chính StartCom Class 1.
...
- Thay vì máy khách thực hiện yêu cầu OCSP, máy chủ sẽ truy xuất định kỳ phản hồi OCSP đã được ký và đánh dấu thời gian từ CA.
...
Để có được kết quả tốt nhất trong quá trình triển khai của riêng bạn, hãy tận dụng tối đa [TLS Session Resumption](#tls-session-resumption) — triển khai, đo lường và tối ưu hóa tỷ lệ thành công của nó. Việc loại bỏ nhu cầu thực hiện các hoạt động mã hóa khóa công khai tốn kém trên mỗi lần bắt tay sẽ làm giảm đáng kể cả chi phí tính toán và độ trễ của TLS; không có lý do gì để dành chu kỳ CPU cho công việc mà bạn không cần phải làm.
...
<figure markdown="span">
    ![](img/6ff953c25c7677f6bb49c6eb0b8737b1.svg)    
...
<figure markdown="span">
    ![](img/bf029cd183682281fcef2c53c9eb85a2.png)
...
Tiếp theo, có thể đặt quyết định về thời điểm tăng kích thước bản ghi và đặt lại nếu kết nối không hoạt động dựa trên các ngưỡng được định cấu hình trước: tăng kích thước bản ghi lên tới 16 KB sau khi XKB dữ liệu được truyền và đặt lại kích thước bản ghi sau Ymili giây thời gian nhàn rỗi.
...
Mỗi kết nối TLS mới yêu cầu trình duyệt phải xác minh chữ ký của chuỗi chứng chỉ đã gửi. Tuy nhiên, có một bước quan trọng nữa mà chúng ta không thể quên: trình duyệt cũng cần xác minh rằng các chứng chỉ chưa bị thu hồi.
...
- Tất cả các yêu cầu đến nguồn gốc phải được gửi qua HTTPS. Điều này bao gồm cả điều hướng và tất cả các yêu cầu tài nguyên phụ cùng nguồn gốc khác—ví dụ: nếu người dùng nhập URL không có tiền tố https, tác nhân người dùng sẽ tự động chuyển đổi URL đó thành yêu cầu https; nếu một trang chứa tham chiếu đến tài nguyên không phải https, tác nhân người dùng sẽ tự động chuyển đổi để yêu cầu phiên bản https.
- Nếu không thể thiết lập kết nối an toàn, người dùng không được phép né tránh cảnh báo và yêu cầu phiên bản HTTP—tức là phiên bản gốc chỉ có HTTPS.
- `max-age` chỉ định thời gian tồn tại của bộ quy tắc HSTS được chỉ định tính bằng giây (ví dụ: max-age=31536000bằng thời gian tồn tại 365 ngày đối với chính sách được quảng cáo).
- `includeSubdomains` cho biết chính sách này sẽ áp dụng cho tất cả các miền phụ của nguồn gốc hiện tại.

HSTS chuyển đổi nguồn gốc thành đích chỉ có HTTPS và giúp bảo vệ ứng dụng khỏi nhiều cuộc tấn công mạng thụ động và chủ động. Như một phần thưởng bổ sung, nó cũng cung cấp khả năng tối ưu hóa hiệu suất tuyệt vời bằng cách loại bỏ nhu cầu chuyển hướng HTTP sang HTTPS: máy khách tự động viết lại tất cả các yêu cầu đến nguồn gốc an toàn trước khi chúng được gửi đi!

!!! note "Note"
    Hãy đảm bảo kiểm tra kỹ lưỡng việc triển khai TLS của bạn trước khi bật HSTS. Sau khi chính sách được lưu trong bộ nhớ đệm của máy khách, việc không thương lượng được kết nối TLS sẽ dẫn đến lỗi cứng—tức là người dùng sẽ thấy trang lỗi của trình duyệt và sẽ không được phép tiếp tục. Hành vi này là một lựa chọn thiết kế rõ ràng và cần thiết để ngăn chặn kẻ tấn công mạng lừa máy khách truy cập trang web của bạn mà không có HTTPS.

!!! info "Info"
    Danh sách tải trước HSTS
    
    Cơ chế HSTS để lại yêu cầu đầu tiên đến một nguồn không được bảo vệ khỏi các cuộc tấn công chủ động—ví dụ: một bên độc hại có thể hạ cấp yêu cầu của máy khách và ngăn không cho máy khách đăng ký chính sách HSTS. Để giải quyết vấn đề này, hầu hết các trình duyệt đều cung cấp một cơ chế "danh sách tải trước HSTS" riêng biệt cho phép một nguồn yêu cầu được đưa vào danh sách các trang web hỗ trợ HSTS đi kèm với trình duyệt.

    Khi bạn đã tự tin vào việc triển khai HTTPS của mình, hãy cân nhắc gửi trang web của bạn tới danh sách tải trước HSTS thông qua [hstspreload.appspot.com](https://hstspreload.appspot.com/).

### Enable HTTP Public Key Pinning - HPKP
> Bật Ghim khóa công khai HTTP (HPKP)

Một trong những thiếu sót của hệ thống hiện tại—như đã thảo luận trong [Chain of Trust and Certificate Authorities](https://hpbn.co/transport-layer-security-tls/#chain-of-trust-and-certificate-authorities) —là sự phụ thuộc của chúng ta vào một số lượng lớn các Certificate Authorities (CA) đáng tin cậy. Một mặt, điều này rất tiện lợi, vì nó có nghĩa là chúng ta có thể có được một chứng chỉ hợp lệ từ một nhóm lớn các thực thể. Tuy nhiên, nó cũng có nghĩa là bất kỳ thực thể nào trong số những thực thể này cũng có thể cấp một chứng chỉ hợp lệ cho nguồn gốc của chúng ta và bất kỳ nguồn gốc nào khác mà không cần sự đồng ý rõ ràng của họ.

!!! note "Note"
    Vụ xâm phạm cơ quan cấp chứng chỉ DigiNotar là một trong số nhiều ví dụ điển hình về việc kẻ tấn công có thể cấp và sử dụng chứng chỉ giả nhưng hợp lệ đối với hàng trăm trang web có uy tín.

Ghim khóa công khai cho phép trang web gửi tiêu đề HTTP hướng dẫn trình duyệt ghi nhớ ("ghim") một hoặc nhiều chứng chỉ trong chuỗi chứng chỉ của nó. Bằng cách đó, nó có thể xác định phạm vi chứng chỉ hoặc tổ chức phát hành nào sẽ được trình duyệt chấp nhận trong các lần truy cập tiếp theo:

Nguồn gốc có thể ghim chứng chỉ lá của nó. Đây là chiến lược an toàn nhất vì về cơ bản, bạn đang mã hóa cứng một tập hợp nhỏ các chữ ký chứng chỉ cụ thể mà trình duyệt sẽ chấp nhận.

Nguồn gốc có thể ghim một trong các chứng chỉ gốc vào chuỗi chứng chỉ. Ví dụ: nguồn gốc có thể ghim chứng chỉ trung gian của CA của nó, điều này cho trình duyệt biết rằng, đối với nguồn gốc cụ thể này, nó chỉ nên tin cậy các chứng chỉ được ký bởi tổ chức phát hành chứng chỉ cụ thể đó.

Việc chọn chiến lược phù hợp để ghim chứng chỉ nào, cung cấp bao nhiêu bản sao lưu, thời lượng và các tiêu chí khác để triển khai HPKP là quan trọng, mang nhiều sắc thái và nằm ngoài phạm vi thảo luận của chúng tôi. Hãy tham khảo công cụ tìm kiếm yêu thích của bạn hoặc chuyên gia bảo mật địa phương của bạn để biết thêm thông tin.

!!! note "Note"
    HPKP cũng hiển thị chế độ "chỉ báo cáo" không thực thi mã pin được cung cấp nhưng có thể báo cáo các lỗi được phát hiện. Đây có thể là bước đầu tiên tuyệt vời để xác thực việc triển khai của bạn và đóng vai trò là cơ chế phát hiện các vi phạm.

### Update Site Content to HTTPS
> Cập nhật nội dung trang web thành HTTPS

Để có được sự đảm bảo về hiệu suất và bảo mật tốt nhất, điều quan trọng là trang web phải thực sự sử dụng HTTPS để tìm nạp tất cả tài nguyên của nó. Nếu không, chúng ta sẽ gặp phải một số vấn đề có thể ảnh hưởng đến cả hai hoặc tệ hơn là phá vỡ trang web:

Nội dung "hoạt động" hỗn hợp (ví dụ như tập lệnh và bảng định kiểu được phân phối qua HTTP) sẽ bị trình duyệt chặn và có thể làm hỏng chức năng của trang web.

Nội dung "thụ động" hỗn hợp (ví dụ: hình ảnh, video, âm thanh, v.v., được gửi qua HTTP) sẽ được tìm nạp nhưng sẽ cho phép kẻ tấn công quan sát và suy ra hoạt động của người dùng, đồng thời làm giảm hiệu suất bằng cách yêu cầu thêm kết nối và bắt tay.

Kiểm tra nội dung của bạn và cập nhật tài nguyên và liên kết của bạn, bao gồm nội dung của bên thứ ba, để sử dụng HTTPS. Cơ chế [Chính sách bảo mật nội dung](https://hpbn.co/csp) (CSP) có thể giúp ích rất nhiều ở đây, cả để xác định vi phạm HTTPS và thực thi các chính sách mong muốn.
...
```
1. Yêu cầu trình duyệt nâng cấp tất cả các yêu cầu (của riêng và bên thứ ba) lên HTTPS.
1. Yêu cầu trình duyệt báo cáo mọi hành vi vi phạm không phải HTTPS tới điểm cuối được chỉ định.

!!! note "Note"
    CSP cung cấp một cơ chế có thể cấu hình cao để kiểm soát tài sản nào được phép sử dụng, cách thức và nơi chúng có thể được lấy. Hãy sử dụng các khả năng này để bảo vệ trang web và người dùng của bạn.

### Performance Checklist
Danh sách kiểm tra hiệu suất

Với tư cách là nhà phát triển ứng dụng, chúng tôi được bảo vệ khỏi hầu hết sự phức tạp của giao thức TLS—máy khách và máy chủ thay mặt chúng tôi thực hiện hầu hết các công việc khó khăn. Tuy nhiên, như chúng ta đã thấy trong chương này, điều này không có nghĩa là chúng ta có thể bỏ qua các khía cạnh hiệu suất của việc phân phối ứng dụng qua TLS. Việc điều chỉnh máy chủ của chúng tôi để kích hoạt các tối ưu hóa TLS quan trọng và định cấu hình ứng dụng của chúng tôi để cho phép khách hàng tận dụng các tính năng đó sẽ mang lại lợi ích cao: bắt tay nhanh hơn, giảm độ trễ, đảm bảo bảo mật tốt hơn, v.v.

Với suy nghĩ đó, đây là danh sách kiểm tra ngắn để đưa vào chương trình nghị sự:

- Nhận hiệu suất tốt nhất từ ​​TCP; xem Tối ưu hóa cho TCP .
- Nâng cấp thư viện TLS lên bản phát hành mới nhất và (xây dựng lại) máy chủ dựa trên chúng.
- Kích hoạt và định cấu hình bộ nhớ đệm phiên và tiếp tục không trạng thái.
- Theo dõi tỷ lệ truy cập bộ nhớ đệm phiên của bạn và điều chỉnh cấu hình cho phù hợp.
- Cấu hình mã hóa bảo mật chuyển tiếp để bật TLS False Start.
- Kết thúc phiên TLS gần người dùng hơn để giảm thiểu độ trễ khứ hồi.
- Sử dụng kích thước bản ghi TLS động để tối ưu hóa độ trễ và thông lượng.
- Kiểm tra và tối ưu hóa quy mô chuỗi chứng chỉ của bạn.
- Cấu hình ghim OCSP.
- Cấu hình HSTS và HPKP.
- Định cấu hình chính sách CSP.
- Kích hoạt HTTP/2; xem HTTP/2 .

## Testing and Verification
> Kiểm tra và xác minh

Cuối cùng, để xác minh và kiểm tra cấu hình của mình, bạn có thể sử dụng một dịch vụ trực tuyến, chẳng hạn như Kiểm tra máy chủ SSL Qualys để quét máy chủ công cộng của bạn để tìm các lỗi bảo mật và cấu hình phổ biến. Ngoài ra, bạn nên tự làm quen với opensslgiao diện dòng lệnh, giao diện này sẽ giúp bạn kiểm tra cục bộ toàn bộ quá trình bắt tay và cấu hình máy chủ của mình.
...
1. Khách hàng đã hoàn tất việc xác minh chuỗi chứng chỉ đã nhận.
1. Đã nhận chuỗi chứng chỉ (hai chứng chỉ).
1. Kích thước của chuỗi chứng chỉ đã nhận.
1. Mã định danh phiên đã phát hành cho sơ yếu lý lịch TLS có trạng thái.

Trong ví dụ trước, chúng tôi kết nối với igvita.com trên cổng TLS mặc định (443) và thực hiện bắt tay TLS. Do s_clientkhông đưa ra giả định nào về chứng chỉ gốc đã biết nên chúng tôi chỉ định thủ công đường dẫn đến chứng chỉ gốc, tại thời điểm viết bài, là Cơ quan cấp chứng chỉ StartSSL cho miền ví dụ. Trình duyệt của bạn đã có chứng chỉ gốc chung và do đó có thể xác minh chuỗi nhưng s_clientkhông đưa ra giả định nào như vậy. Hãy thử bỏ qua chứng chỉ gốc và bạn sẽ thấy lỗi xác minh trong nhật ký.

Kiểm tra chuỗi chứng chỉ cho thấy máy chủ đã gửi hai chứng chỉ, tổng cộng lên tới `3.571 byte`. Ngoài ra, chúng ta có thể thấy các biến phiên TLS đã thương lượng—giao thức đã chọn, mật mã, khóa—và chúng ta cũng có thể thấy máy chủ đã cấp một mã định danh phiên cho phiên hiện tại, có thể được tiếp tục trong tương lai.